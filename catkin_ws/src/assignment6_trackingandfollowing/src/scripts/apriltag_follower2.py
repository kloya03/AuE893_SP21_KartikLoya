...
class Apriltag_follower(object):
    ...
    def callback(self, data):
        ...
        try:
            x_diff = data.detections[0].pose.pose.pose.position.x - width/2       #compensate the width to determine the right x difference
            ...
            Kp_depth_diff = 0.4                                                 #the proportional value
            Kp_x_diff = 3.5                                                    #the proportional value

            #the velocity message
            if depth_diff < 0.6:                                                #the robot will maintain distance of 1 cm
                vel_msg.linear.x = 0
            else:
